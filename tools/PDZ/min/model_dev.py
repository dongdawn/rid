...
def _main () :
    ...
    data2 = np.loadtxt('x.cry.out')[:,:-2]
    devi1 = compute_model_dev(models, data)
    devi2 = compute_model_dev(models, data2)

    np.savetxt('devi.cry.dat', np.concatenate([devi1.reshape([-1,1]),devi2.reshape([-1,1])],axis=1))
