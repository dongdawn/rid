...
class Walker(object):
    def __init__(self, fd_dih, fd_dist, nw, sess):
        ...
        self._position1 = np.concatenate((np.random.uniform(size=self._shape1_) * np.pi * 2, self._position_dist0,self._position_dist1,self._position_dist2), axis=1)
    ...
    def sample(self, compute_ef, inter_step=1):
        ...
        for _ in range(inter_step):
            dist1=self._position[:,self._full_dim_dih+1].reshape(-1,1)+np.random.normal(scale=self._move_scale*0.4,size=self._shape11)
            dist1[dist1<0.3]=0.37+np.random.normal(scale=0.02)
            dist1[dist1>3.79]=3.72+np.random.normal(scale=0.02)

            dist0=self._position[:,self._full_dim_dih].reshape(-1,1)+np.random.normal(scale=self._move_scale*0.4,size=self._shape11)
            ...
            dist2=self._position[:,self._full_dim_dih+2].reshape(-1,1)+np.random.normal(scale=self._move_scale*0.4,size=self._shape11)
            ...
            position_new =np.concatenate(( np.mod(self._position[:,0:self._full_dim_dih] + np.random.normal(scale=self._move_scale,size=self._shape1), 2*np.pi),dist0,dist1,dist2), axis=1)
