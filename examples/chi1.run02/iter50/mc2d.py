...
def _main () :
    ...
    parser.add_argument("-fd", "--full_dimension", default = 3, type=int, 
    ...
    pp_hist = np.zeros((1, len(xx), len(yy)))
    ...
    with tf.Session(graph = graph) as sess:        
        ...
        for ii in range(ns):
            pp, ee, ff = walker.sample(compute_ef)
            pp_hist_new = my_hist2d(pp, xx, yy, delta, cv1, cv2)
            ...
            if np.mod(ii,10) == 0:
                ...
                zz *= f_cvt/4.184
                zz = zz - np.min(zz)
                np.savetxt("all_angle2d%d.dat" %ii,zz[0])
