...
def _main () :
    ...
    parser.add_argument("-cv", "--cv_index", default=0, type=int, 
                        help="cv index")
    ...
    yy = pylab.linspace(0,2* np.pi, 25)
    pp_hist = np.zeros((1, len(xx)))
    delta = 2.0 * np.pi / 25.0
    ...
    with tf.Session(graph = graph) as sess:        
        ...
        for ii in range(ns):
            pp, ee, ff = walker.sample(compute_ef)
            pp_hist_new = my_hist1d_1cv(pp, xx, delta, cv)
            pp_hist = (pp_hist * ii + pp_hist_new) / (ii+1)
            if np.mod(ii,10000) == 0:
                zz = -np.log(pp_hist+1e-6)/beta
                zz *= f_cvt/4.184   ##kcal
                zz = zz - np.min(zz)
                np.savetxt(fp,zz)
