...
def value_array_0 (sess, ngrid) :
    ...
    for i in range(len(xx)):
        ...
        zz[i*(ngrid+1)] = (kbT) * np.log(np.sum(delta2 * np.exp(-beta * ve)))
...
def value_array_1 (sess, ngrid) :
    ...
    for i in range(len(xx)):
        ...
        for j in range(len(yy)):
            ve = test_e (sess, np.concatenate((zero_grid + xx[i]), axis = 1))
...
def _main () :
    ...
    #avg2 -= np.max(avg2)
    #avg3 -= np.max(avg2)
    print_array(args.output, xx, avg0)
