...
def value_array_12 (sess, ngrid) :
    ...
    for i in range(len(xx)):
        ...
        for j in range(len(yy)):
            ve = test_e (sess, np.concatenate((np.reshape(my_grid[:,0], [-1,1]),zero_grid + xx[i], zero_grid + yy[j]), axis = 1))
...
def value_array_02 (sess, ngrid) :
    ...
    for i in range(len(xx)):
        print("computing grid: %d" % i)
        for j in range(len(yy)):
            ve = test_e (sess, np.concatenate((zero_grid + xx[i],np.reshape(my_grid[:,0], [-1,1]), zero_grid + yy[j]), axis = 1))
            zz[i*(ngrid+1)+j] = (kbT) * np.log(np.sum(delta2 * np.exp(-beta * ve)))
...
def value_array_01 (sess, ngrid) :
    ...
    for i in range(len(xx)):
        print("computing grid: %d" % i)
        for j in range(len(yy)):
            ve = test_e (sess, np.concatenate((zero_grid + xx[i], zero_grid + yy[j], np.reshape(my_grid[:,0], [-1,1])), axis = 1))
            zz[i*(ngrid+1)+j] = (kbT) * np.log(np.sum(delta2 * np.exp(-beta * ve)))
