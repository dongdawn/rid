...
def make_enhc (iter_index, 
    ...
    init_upper_wall = jdata["upper_wall"]
    ...
    for walker_idx in range (numb_walkers) :
        ...
        upper_wall=float(init_upper_wall)+0.15*walker_idx
...
###########################################
            ...
            if num_of_cluster < num_of_cluster_threshhold:
                ...
                enhc_trust_lvl_1 = jdata["bias_trust_lvl_1"]
                enhc_trust_lvl_2 = enhc_trust_lvl_1+1
            if enhc_trust_lvl_1>jdata["bias_trust_lvl_1"]*20:
                enhc_trust_lvl_1 = jdata["bias_trust_lvl_1"]
                enhc_trust_lvl_2 = enhc_trust_lvl_1+1
            pre_upper_wall=np.loadtxt(prev_enhc_path+'upper_wall.dat')
            if pre_upper_wall <1.4:
                upper_wall=3.55
            else:
                upper_wall=pre_upper_wall-0.15
        ...
        np.savetxt(walker_path+'upper_wall.dat', [upper_wall] , fmt = '%.6f')
        replace(plm_conf, "UPPER_WALLS ARG=d1,d2,d3 AT=[^ ]* ", ("UPPER_WALLS ARG=d1,d2,d3 AT=%f,%f,%f " %(upper_wall,upper_wall,upper_wall)))
        ...
        replace(plm_bf_conf, "UPPER_WALLS ARG=d1,d2,d3 AT=[^ ]* ", ("UPPER_WALLS ARG=d1,d2,d3 AT=%f,%f,%f " %(upper_wall,upper_wall,upper_wall)))
        if len(graph_list) == 0 :
