    ...
    def sample(self, compute_ef, inter_step=1):
        ...
        for ii in self._models:
            ...
            with tf.Session(graph = graph) as sess:
                temp_energy, temp_force = compute_ef(sess, self._position)
                energys.append(temp_energy)
                forces.append(temp_force)
        self._energy, self._force = np.mean(energys,axis=0),np.mean(forces,axis=0)
        ...
        for _ in range(inter_step):
            ...
            for ii in self._models:
                ...
                with tf.Session(graph = graph) as sess:
                    ...
                    energys.append(temp_energy)
                    forces.append(temp_force)
            energy_new, force_new = np.mean(energys,axis=0),np.mean(forces,axis=0)
            ...
            #print('========energy_new========\n')
            #print(energy_new)
            energy_new[forces_walker_std>2]=energy_new[forces_walker_std>2]+forces_walker_std[forces_walker_std>2].reshape(-1,1)
